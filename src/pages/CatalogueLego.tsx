...
import { legoCatalogueProducts, getStockBadgeColor, LegoProduct } from '@/lib/lego-catalogue-data';
...
import { ShoppingCart, Plus, Minus, Trash2, Send, Search, Package, Barcode, ArrowLeft, Image, Pencil, RotateCcw } from 'lucide-react';
...
const editFields: EditField[] = [
  ...
  { key: 'prixAchatHT', label: 'Prix Achat HT (€)', type: 'number' },
  { key: 'pvmc', label: 'PVMC (€)', type: 'number' },
];
...
const CatalogueLego = () => {
  ...
  const [editProduct, setEditProduct] = useState<LegoProduct | null>(null);

  const { products, isAdmin, updateProduct, deleteProduct, resetToDefault } = useCatalogueAdminEdit<LegoProduct>(
    'lego', legoCatalogueProducts, 'id'
  );
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (stockFilter === 'inStock') return matchesSearch && stockNum > 0;
    if (stockFilter === 'outOfStock') return matchesSearch && stockNum === 0;
    return matchesSearch;
  });

  const addToCart = (product: LegoProduct) => {
    setCart(prev => {
  ...
  const submitOrder = async () => {
    ...
    try {
      const orderLines = cart.map(item => `• ${item.quantity}x ${item.product.designation} (Réf: ${item.product.reference}) - ${(item.product.prixAchatHT * item.quantity).toFixed(2)}€ HT`).join('\n');
      const orderSummary = `📦 COMMANDE CATALOGUE LEGO\n═══════════════════════════════════\n${orderLines}\n═══════════════════════════════════\n💰 TOTAL HT: ${totalHT.toFixed(2)}€\n📊 Nombre d'articles: ${totalItems}\n═══════════════════════════════════\nDétails techniques:\n${cart.map(item => `[${item.product.id}] EAN: ${item.product.ean} | Réf: ${item.product.reference} | Qté: ${item.quantity}`).join('\n')}`;
      const { data: ticket, error: ticketError } = await supabase.from('tickets').insert({
        user_id: user.id, subject: `🧱 COMMANDE LEGO - ${totalItems} article(s) - ${totalHT.toFixed(2)}€ HT`,
        category: 'catalogue_exclusif', subcategory: 'lego', priority: 'high', status: 'open'
      }).select().single();
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold flex items-center gap-2">
                <Package className="w-6 h-6 text-red-500" /> Catalogue LEGO
              </h1>
            </div>
            {isAdmin && (
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <Dialog open={isCartOpen} onOpenChange={setIsCartOpen}>
              <DialogTrigger asChild>
                <Button size="lg" className="relative bg-red-500 hover:bg-red-600">
              ...
              <DialogContent className="max-w-2xl max-h-[80vh]">
                        ...
                        <Card key={item.product.id} className="p-4">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground">Réf: {item.product.reference} • {item.product.prixAchatHT.toFixed(2)}€ HT/u</p>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => updateQuantity(item.product.id, 1)}><Plus className="w-4 h-4" /></Button>
                            ...
                            <p className="font-bold w-24 text-right">{(item.product.prixAchatHT * item.quantity).toFixed(2)}€</p>
                          </div>
                        </Card>
                      ))}
                    </div>
                ...
                {cart.length > 0 && (
                  <DialogFooter className="flex-col sm:flex-row gap-4 border-t pt-4">
                    ...
                    <Button size="lg" onClick={submitOrder} disabled={isSubmitting} className="gap-2 bg-red-500 hover:bg-red-600">
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input placeholder="Rechercher par nom, EAN, référence ou ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
                </div>
                  ...
                  <TableBody>
                    {filteredProducts.map((product) => (
                      <TableRow key={product.id} className="hover:bg-muted/50">
                        <TableCell>
                          {product.imageUrl && product.imageUrl !== 'en attente' ? (
                            <img src={product.imageUrl} alt={product.designation} className="w-12 h-12 object-contain rounded"
                              onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                          ) : (
                            <div className="w-12 h-12 bg-muted rounded flex items-center justify-center"><Image className="w-6 h-6 text-muted-foreground" /></div>
                          )}
                        </TableCell>
                        ...
                        <TableCell><Badge variant="outline" className={getStockBadgeColor(product.stock)}>{product.stock}</Badge></TableCell>
                        <TableCell className="text-right font-bold">{product.prixAchatHT.toFixed(2)}€</TableCell>
                        <TableCell className="text-right text-muted-foreground hidden sm:table-cell">{product.pvmc.toFixed(2)}€</TableCell>
                        <TableCell>
                          <Button size="sm" onClick={() => addToCart(product)} className="gap-1 bg-red-500 hover:bg-red-600">
