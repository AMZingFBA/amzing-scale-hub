...
import { ShoppingCart, Plus, Minus, Trash2, Send, Search, Package, Barcode, ArrowLeft, Image } from 'lucide-react';
...
const CatalogueLego = () => {
  ...
  const filteredProducts = legoCatalogueProducts.filter(product => {
  ...
  const submitOrder = async () => {
    ...
    try {
      const orderLines = cart.map(item => 
        `• ${item.quantity}x ${item.product.designation} (Réf: ${item.product.reference}) - ${(item.product.prixAchatHT * item.quantity).toFixed(2)}€ HT`
      ).join('\n');

      const orderSummary = `
📦 COMMANDE CATALOGUE LEGO
═══════════════════════════════════
...
${cart.map(item => `[${item.product.id}] EAN: ${item.product.ean} | Réf: ${item.product.reference} | Qté: ${item.quantity}`).join('\n')}
      ...
      const { data: ticket, error: ticketError } = await supabase
        ...
        .insert({
          user_id: user.id,
          subject: `🧱 COMMANDE LEGO - ${totalItems} article(s) - ${totalHT.toFixed(2)}€ HT`,
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <p className="text-muted-foreground mt-1">
                {legoCatalogueProducts.length} produits disponibles
              </p>
            </div>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Réf: {item.product.reference} • {item.product.prixAchatHT.toFixed(2)}€ HT/u
                              </p>
                            </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input
                    placeholder="Rechercher par nom, EAN, référence ou ID..."
                    value={searchTerm}
