...
import { djiCatalogueProducts, DJIProduct } from '@/lib/dji-catalogue-data';
...
const editFields: EditField[] = [
  ...
  { key: 'stock', label: 'Stock (ex: EN STOCK, MI MARS)', type: 'text' },
...
const getStockBadgeColor = (stock: string) => {
  if (stock === 'EN STOCK') return 'border-green-500 text-green-600';
...
const CatalogueDJI = () => {
  ...
  const [editProduct, setEditProduct] = useState<DJIProduct | null>(null);

  const { products, isAdmin, updateProduct, deleteProduct, resetToDefault } = useCatalogueAdminEdit<DJIProduct>(
    'dji', djiCatalogueProducts, 'id'
  );
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      const orderSummary = `📦 COMMANDE CATALOGUE DJI\n═══════════════════════════════════\n${orderLines}\n═══════════════════════════════════\n💰 TOTAL HT: ${totalHT.toFixed(2)}€\n📊 Nombre d'articles: ${totalItems}\n═══════════════════════════════════\nDétails techniques:\n${cart.map(item => `[${item.product.id}] EAN: ${item.product.ean} | Qté: ${item.quantity} | Réf: ${item.product.reference}`).join('\n')}`;

      const { data: ticket, error: ticketError } = await supabase.from('tickets').insert({
        user_id: user.id, subject: `🎯 COMMANDE DJI - ${totalItems} article(s) - ${totalHT.toFixed(2)}€ HT`,
        category: 'catalogue_exclusif', subcategory: 'dji', priority: 'high', status: 'open'
      }).select().single();
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold flex items-center gap-2">
                <Package className="w-6 h-6 text-primary" /> Catalogue DJI
              </h1>
            </div>
            {isAdmin && (
                        ...
                        <Card key={item.product.id} className="p-4">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground">Réf: {item.product.id} • {item.product.prixAchatHT.toFixed(2)}€ HT/u</p>
                            </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant={stockFilter === 'soon' ? 'default' : 'outline'} size="sm" onClick={() => setStockFilter('soon')}>Bientôt</Button>
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  <TableBody>
                    {filteredProducts.map((product) => (
                      <TableRow key={product.id} className="hover:bg-muted/50">
                        ...
                        <TableCell>
                          <Badge variant="outline" className={getStockBadgeColor(product.stock)}>{product.stock}</Badge>
                        </TableCell>
