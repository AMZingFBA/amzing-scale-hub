...
import { bhDistributionCatalogueProducts, BHDistributionProduct } from '@/lib/bh-distribution-catalogue-data';
...
const CatalogueBHDistribution = () => {
  ...
  const [editProduct, setEditProduct] = useState<BHDistributionProduct | null>(null);

  const { products, isAdmin, updateProduct, deleteProduct, resetToDefault } = useCatalogueAdminEdit<BHDistributionProduct>(
    'bh-distribution', bhDistributionCatalogueProducts, 'ean'
  );
  ...
  const addToCart = (product: BHDistributionProduct) => {
    setCart(prev => {
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      const orderSummary = `
📦 COMMANDE CATALOGUE BH DISTRIBUTION
═══════════════════════════════════
${orderLines}
═══════════════════════════════════
...
${cart.map(item => `EAN: ${item.product.ean} | Qté: ${item.quantity}`).join('\n')}
      ...
      const { data: ticket, error: ticketError } = await supabase
        ...
        .insert({
          user_id: user.id,
          subject: `📦 COMMANDE BH DISTRIBUTION - ${totalItems} article(s) - ${totalTTC.toFixed(2)}€ TTC`,
          category: 'catalogue_exclusif',
          subcategory: 'bh-distribution',
          priority: 'high',
          status: 'open'
        })
        .select()
...
export default CatalogueBHDistribution;
