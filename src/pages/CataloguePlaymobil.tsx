...
import { playmobilCatalogueProducts, getStockStatus, getStockBadgeColor, PlaymobilProduct } from '@/lib/playmobil-catalogue-data';
import { ShoppingCart, Plus, Minus, Trash2, Send, Search, Package, Barcode, ArrowLeft, ExternalLink } from 'lucide-react';
...
const CataloguePlaymobil = () => {
  ...
  const filteredProducts = playmobilCatalogueProducts.filter(product => {
  ...
  const submitOrder = async () => {
    ...
    try {
      const orderLines = cart.map(item => 
        `• ${item.quantity}x ${item.product.designation} (Réf: ${item.product.id}) - ${(item.product.prixNetHT * item.quantity).toFixed(2)}€ HT`
      ).join('\n');

      const orderSummary = `
📦 COMMANDE CATALOGUE PLAYMOBIL
═══════════════════════════════════
...
${cart.map(item => `[${item.product.id}] EAN: ${item.product.ean} | Qté: ${item.quantity} | Réf: ${item.product.reference}`).join('\n')}
      ...
      const { data: ticket, error: ticketError } = await supabase
        ...
        .insert({
          user_id: user.id,
          subject: `🎯 COMMANDE PLAYMOBIL - ${totalItems} article(s) - ${totalHT.toFixed(2)}€ HT`,
          category: 'catalogue_exclusif',
          subcategory: 'playmobil',
          priority: 'high',
          status: 'open'
        })
        .select()
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <p className="text-muted-foreground mt-1">
                {playmobilCatalogueProducts.length} produits disponibles
              </p>
            </div>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Réf: {item.product.id} • {item.product.prixNetHT.toFixed(2)}€ HT/u
                              </p>
                            </div>
                            ...
                            <p className="font-bold w-24 text-right">
                              {(item.product.prixNetHT * item.quantity).toFixed(2)}€
                            </p>
                          </div>
                        </Card>
                      ))}
                    ...
                    {filteredProducts.map((product) => (
                      <TableRow key={product.id} className="hover:bg-muted/50">
                          ...
                          {product.imageUrl ? (
                            <a href={product.imageUrl} target="_blank" rel="noopener noreferrer">
                              <img 
                                ...
                                className="w-12 h-12 object-cover rounded border hover:opacity-75 transition-opacity"
                                onError={(e) => {
                        ...
                        <TableCell className="text-right font-bold">
                          {product.prixNetHT.toFixed(2)}€
                        </TableCell>
                        <TableCell className="text-right text-muted-foreground hidden sm:table-cell">
                          {product.pvmcTTC.toFixed(2)}€
                        </TableCell>
                        <TableCell>
                          <Button
                            size="sm"
