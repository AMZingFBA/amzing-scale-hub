...
interface AuchanProduct {
  ...
  auchan_url: string | null;
  chart_url: string | null;
}

export default function ProduitsAuchan() {
  ...
  const fetchProducts = useCallback(async (silent = false) => {
    ...
    setIsSyncing(true);
    try {
      const { data, error } = await supabase.functions.invoke('sync-auchan-from-sheets');
      ...
      if (data?.success) {
        ...
        if (!silent) toast.success(`${data.count} produits Auchan chargés`);
      } else {
        throw new Error(data?.error || 'Fetch failed');
      }
    } catch (error: any) {
      console.error('Auchan fetch error:', error);
      if (!silent && error?.name !== 'AbortError') {
        toast.error('Erreur lors du chargement des produits Auchan');
      }
    } finally {
      setIsSyncing(false);
      setIsLoading(false);
  ...
  useEffect(() => {
    if (!authLoading && user && (isVIP || isAdmin)) {
      ...
      // Mark auchan alerts as read
      supabase.rpc('mark_product_find_alerts_as_read', { source_filter: 'Auchan' });
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="text-center flex-1">
              ...
              <p className="text-muted-foreground text-sm">Alertes Auchan en temps réel</p>
            </div>
            <Button
              variant="outline"
              size="icon"
        ...
        {products.length === 0 ? (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <h3 className="text-xl font-semibold mb-2">Aucun produit Auchan</h3>
              <p className="text-muted-foreground">Les produits Auchan apparaîtront ici.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-6">
            {products.map((product) => (
              <AuchanProductCard key={product.id} product={product} onCopy={copyToClipboard} onDelete={isAdmin ? deleteProduct : undefined} />
            ))}
          </div>
        )}
      </div>
...
function AuchanProductCard({ product, onCopy, onDelete }: { product: AuchanProduct; onCopy: (text: string) => void; onDelete?: (id: string) => void }) {
    ...
    <Card className="overflow-hidden border border-border/60 hover:border-primary/40 transition-all duration-300 hover:shadow-lg">
      <div className="px-4 py-2.5 border-b bg-muted/30 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-primary">Auchan</span>
        </div>
        <div className="flex items-center gap-2">
          {product.asin && (
            <button
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 space-y-3 min-w-0">
            ...
            <div className="flex gap-2">
              ...
              {product.auchan_url && (
                <Button variant="outline" size="sm" className="h-8 text-xs" asChild>
                  <a href={product.auchan_url} target="_blank" rel="noopener noreferrer" className="gap-1.5">
                    <ExternalLink className="w-3.5 h-3.5" />
                    Auchan
                  </a>
                </Button>
