...
class SellerAmpClient:
    ...
    async def lookup_batch(self, asins: list[str], keepa_mp: int) -> dict:
        """Lookup a batch of ASINs on a specific marketplace. Returns dict[asin -> raw data]."""
        payload = {
        ...
        # SellerAmp returns null body when session expired, ASIN invalid, or rate-limited.
        # Attempt a single re-login + retry on the second pass, then give up gracefully.
        ...
        for asin in asins:
            ...
            data_map[asin] = {
                ...
                'prix_fba_local': prix_fba / 100 if prix_fba > 0 else None,
                'ref_pct': ref_pct,
