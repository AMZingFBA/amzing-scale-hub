...
const SHEET_ID = "1faceHvZIjum7JCWsDEoA1ZWBuHepcivYL5oc3ERGsrM";
const SHEET_GID = "1469094466";
...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        ...
        products.push({
          ...
          chart_url: chartUrl,
          auchan_url: getCellValue(cells[26])?.trim() || null,
        });
      } catch (e) {
    ...
    console.log(`✅ Auchan: ${products.length} products loaded from ${rows.length} rows`);

    // --- Insert individual product alerts into product_find_alerts ---
    if (products.length > 0) {
      try {
        ...
        if (adminRole) {
          // Get ALL existing Auchan alerts to avoid duplicates (paginate)
          ...
          while (true) {
            const { data: existingBatch, error: existingErr } = await supabase
              ...
              .eq('source_name', 'Auchan')
              .range(from, from + pageSize - 1);

            if (existingErr) {
              console.error('existing Auchan alerts fetch error:', existingErr);
              break;
            }
          ...
          if (newAlerts.length > 0) {
            const inserts = newAlerts.map(p => ({
              ...
              source_name: 'Auchan',
              source_url: p.auchan_url || p.amazon_url || null,
            ...
            if (insertErr) {
              ...
              console.log(`📢 ${newAlerts.length} new Auchan product alerts created`);
            }
          } else {
            console.log('⏭️ No new Auchan products to alert');
          }
        }
      } catch (alertErr) {
    ...
    console.error("❌ Auchan sync error:", error);
    return new Response(
